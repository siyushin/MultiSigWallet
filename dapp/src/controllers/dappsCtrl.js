(
  function () {
    angular
      .module('multiSigWeb')
      .controller('dappsCtrl', function ($rootScope, $scope, $uibModal, $window, CommunicationBus, Token, Utils, Wallet, Web3Service) {
        ...
        $scope.go = function (url) {
          ...
          setTimeout(() => {
            ...
            w.test = $scope.wallets;
            w.test1 = window.web3.currentProvider;
            console.log("给完了值。", $scope.wallets)
          }, 10000);

          // setTimeout(() => {
          //   console.log("发送之前", { ...window.web3.currentProvider });
          //   w.postMessage({ ...window.web3.currentProvider }, "http://localhost:3000");
          // }, 10000);
        }
      });
  }
